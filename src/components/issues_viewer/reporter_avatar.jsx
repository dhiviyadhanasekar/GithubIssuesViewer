    ...
    render: function(){
      ...
      if(this.props.displayUserName !== false)
        userName = <div className='verysmall row margin_auto_left' style={{paddingTop: 2}}>
                        <div className='ellipsis block margin_10_right testId_username' style={userNameStyle}>
                            {this.props.user.login ? '@' + this.props.user.login : null}
                        </div>
                    </div>;    

      return (
        <a className='flex_center column margin_10_left gray pointer no_underline auto_width testId_reporter_avatar' href={this.props.user.html_url}>
